...
def main():
    ...
    cipher_key = cipherkey(key)
   # plainkey=plainkey(cipherkey)
    # Decryption
    ...
    print("Encrypted Ciphertext is : %r " % cipher_key)
   # print("Decrypted plaintext is  : ", plainkey)
...
def cipherkey(key):
   ...
   # print('PRIVATE:', D)
    ...
    openkey=[E, N]
    secretkey=[D , N]
    openfile=open("open.txt", 'w')
    secretfile= open("secret.txt", 'w')
    openfile.writelines("%s\n" % i for i in openkey)
    secretfile.writelines("%s\n" % i for i in secretkey)
    
    return int_to_msg(r_enc)
...
def DES(text, key, padding, isEncrypt):
    ...
    for block in plaintext8byteBlocks:
        ...
        for i in range(16): # ЛАВИННЫЙ ЭФФЕКТ ДЛЯ ТЕЕКСТААААА!!!!!!!!!!!!!!!!!!!!!!!!!1
...
def generateKeys(key):
    ...
    # 16 rounds of keys 
    for i in range(16): #ЛАВИННЫЙ ЭФФЕКТ ДЛЯ КЛЮЧА!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
